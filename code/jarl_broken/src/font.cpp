...
void Font::disp_text(int x, int y, sstring txt)
{
	...
	if(font != NULL)
	{
		...
		for(int show = 0; txt[show] != '\0'; show++)
		{
			...
			else
			{
				...
				display.apply_surface(X, Y, font, &chars[ascii]/*, txt_surface*/);
				//Move over the width of the character with one pixel of padding
				X += chars[ascii].w + 1;
			}
		}
	}
	//display.apply_surface(X, Y, txt_surface, NULL);
	
}

void Font::load_font(sstring font_file)
{
	font = display.load_img(font_file);
		...
		for(int cols = 0; cols < n_cols; cols++)
		{
			...
			for(int pixel_col = 0; pixel_col < cell_width; pixel_col++)
			{
				...
				for(int pixel_row = 0; pixel_row < cell_height; pixel_row++)
				{
					...
					if(display.get_pixel32(pixel_x, pixel_y, font) != bg_color)
					{
						//Set the x offset
